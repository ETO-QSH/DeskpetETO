...
int main() {
	...
	int atlas_length = 0;
	uint8_t *atlas_bytes = read_file("data/official/spineboy-pma.atlas", &atlas_length);
	spine_atlas atlas = spine_atlas_load_callback((utf8 *) atlas_bytes, "data/official/", load_texture, unload_texture);
	int skeleton_length = 0;
	uint8_t *skeleton_bytes = read_file("data/official/spineboy-pro.skel", &skeleton_length);
